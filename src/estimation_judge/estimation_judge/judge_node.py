...
class EstimationJudgeNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("model_name", config.DEFAULT_MODEL)
    ...
    def _setup_subscriber(self):
        if self.input_mode == "compressed":
            self.create_subscription(CompressedImage, 
                self.get_val("compressed_topic"), self._on_compressed, 10)
        else:
            self.create_subscription(String, 
                self.get_val("image_topic"), self._on_path, 10)
    ...
    def _process_image(self, img_bytes, mime_type):
        prompt = config.get_prompt(self.expected_count, config.ALLOWED_LABELS)
        ...
        type_ids = utils.convert_labels_to_ids(labels, config.LABEL_TO_ID, self.unknown_type_id)
